...
# change the number to match the count of all pairs
image_files = [f for f in bucket.list_blobs(prefix='images/') if f.name.endswith('.jpg')]
image_files=sorted(image_files, key=lambda x: int(x.name.split("-")[1].split(".")[0]))
...
def process_images(img1, img2, i):
    ...
    img1_data = img1.download_as_string()
    img2_data = img2.download_as_string()

    # Open the images
    images = [Image.open(img1_data), Image.open(img2_data)]
